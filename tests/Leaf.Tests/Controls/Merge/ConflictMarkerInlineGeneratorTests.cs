...
public class ConflictMarkerInlineGeneratorTests
{
    ...
    // The marker→rangeIndex lookup that ConflictMarkerInlineGenerator's
    // FindRangeIndexForLine performs is now a thin wrapper over
    // MergeDocument.BuildDisplayMap. End-to-end coverage for the marker
    // pairing semantics — including the post-acceptance "skip resolved
    // conflicts" regression — lives in MergeDisplayMapTests.
}
